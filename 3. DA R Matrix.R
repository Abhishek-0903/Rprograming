...
# To add one more row to a matrix (rbind)
mat2=rbind(mat1,c(26:30))
mat2


# To add one more column to a matrix (cbind)
...
# if - else block  (if - else is a condition) 
num_views=9 # try 9
if(num_views<8){
  print("The show is boaring")
}else{
  print("The show is worth watching")   # else is a keyword
}

# else-if (is a condition)
...
for (i in x)
{
  if(i %% 2 == 0){
    count = count + 1
    values=append(values,i)  # append means adding values 
    #print(i)
  }
}
print(x)
